...
export async function writeV2Report(workspace: string, state: V2RunState): Promise<string> {
  ...
  await writeFile(path, JSON.stringify({
    ...
    reviewerDecision: state.reviewerDecision,
    stageTimeline: state.deliveryStatus.stageResults,
    currentStage: state.deliveryStatus.currentStage,
    completedStages: state.deliveryStatus.completedStages,
    failedStage: state.deliveryStatus.failedStage,

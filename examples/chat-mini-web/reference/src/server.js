...
function mapUpstreamPayload(payload) {
  ...
  try {
    ...
    if (event.type === "error") {
      return buildErrorAction(event.error ?? event);
    }
    if (event.type === "response.failed") {
      return buildErrorAction(event.response?.error ?? event.error ?? event.response ?? event);
    }
    return null;
  } catch {
...
export function extractUpstreamErrorMessage(source) {
  ...
  if (source.error && typeof source.error === "object" && typeof source.error.message === "string" && source.error.message.length > 0) {
    return source.error.message;
  }
  if (source.response && typeof source.response === "object" && source.response.error && typeof source.response.error === "object") {
    if (typeof source.response.error.message === "string" && source.response.error.message.length > 0) {
      return source.response.error.message;
    }
  }
  return "OpenAI streaming error.";
}
...
function extractUpstreamErrorCode(source) {
  ...
  if (source.error && typeof source.error === "object" && typeof source.error.code === "string" && source.error.code.length > 0) {
    return source.error.code;
  }
  if (source.response && typeof source.response === "object" && source.response.error && typeof source.response.error === "object") {
    if (typeof source.response.error.code === "string" && source.response.error.code.length > 0) {
      return source.response.error.code;
    }
  }
  return undefined;
}
